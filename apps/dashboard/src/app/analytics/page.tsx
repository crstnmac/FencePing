      ...
      <div className="flex-1 overflow-auto p-4">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3 mb-6">
          <div className="flex items-center gap-3">
            ...
            <button className="flex items-center gap-2 px-3 py-1.5 text-gray-600 border border-gray-300 rounded-lg hover:bg-gray-50">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-gray-900">{activeDevices} / {totalDevices}</p>
                <p className="text-sm text-green-600 flex items-center mt-2">
                  <ArrowUpRight className="h-4 w-4 mr-1" />
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-gray-900">{totalGeofences}</p>
              ...
              {Object.entries(eventTypes).map(([type, count]) => {
                ...
                return (
                  <div key={type} className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className={`w-4 h-4 rounded-full ${colors[type as keyof typeof colors] || 'bg-gray-500'}`}></div>
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            ...
            {deviceActivity.length > 0 ? (
                ...
                {deviceActivity.slice(-7).map((day, index) => (
                  <div key={day.date} className="flex items-center gap-4">
                    ...
                    <div className="flex-1 flex gap-1">
                      <div
                        className="bg-green-400 rounded-sm h-6 flex items-center justify-center text-white text-xs"
                        style={{ width: `${(day.online / (day.online + day.offline)) * 100}%`, minWidth: '40px' }}
                      >
                        {day.online}
                      </div>
                      <div
                        className="bg-red-400 rounded-sm h-6 flex items-center justify-center text-white text-xs"
                        style={{ width: `${(day.offline / (day.online + day.offline)) * 100}%`, minWidth: '30px' }}
                      >
                        {day.offline}
                      </div>
                    </div>
              ...
              <div className="text-center py-8">
                ...
                <p className="text-sm text-gray-500">No device activity data available for this period</p>
              </div>
            )}
            ...
            <table className="min-w-full">
              <thead>
                <tr className="border-b border-gray-200">
                  <th className="text-left py-3 px-4 text-sm font-medium text-gray-600">Automation</th>
                  <th className="text-left py-3 px-4 text-sm font-medium text-gray-600">Success Rate</th>
                  <th className="text-left py-3 px-4 text-sm font-medium text-gray-600">Total Executions</th>
                  <th className="text-left py-3 px-4 text-sm font-medium text-gray-600">Success/Failed</th>
                      ...
                      <td className="py-4 px-4">
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                          ...
                          {automation.success >= 95 ? (
                            ...
                            <><AlertCircle className="w-3 h-3 mr-1" /> Good</>
                          ) : (
                ...
                <tbody>
                  <tr>
                    <td colSpan={5} className="py-12 text-center">
                      ...
                      <p className="text-sm text-gray-500">No automation performance data available</p>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-lg font-semibold text-gray-900">Top Active Geofences</h3>
            <MapPin className="h-5 w-5 text-gray-400" />
          </div>
          
          <div className="space-y-4">
            {topGeofences.map((geofence, index) => (
              <div key={geofence.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-600">{geofence.geofence_type} • Created {new Date(geofence.created_at).toLocaleDateString()}</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-sm font-medium text-gray-900">{geofence.eventCount} events</p>
            ...
            {topGeofences.length === 0 && (
              <div className="text-center py-8 text-gray-500">
                ...
                <p>No geofence activity data available</p>
              </div>
            )}
          </div>
        </div>
