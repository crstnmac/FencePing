 ...
 * 
 * Usage: node test-automation-flow.mjs
 */
...
class AutomationFlowTester {
  ...
  async setupTestData() {
    ...
    `, [randomUUID(), this.testData.account.id, 'Test ESP32 Device', deviceKey, true]);
    ...
    const geofenceResult = await this.dbPool.query(`
      INSERT INTO geofences (id, account_id, name, type, geom, radius_m, active, created_at)
      VALUES ($1, $2, $3, $4, ST_Buffer(ST_SetSRID(ST_MakePoint($5, $6), 4326)::geography, $7)::geometry, $8, $9, NOW())
    ...
    `, [
      ...
      100,       // 100 meter radius for buffer
      100,       // radius_m field
      true       // active
    ]);
    ...
    const ruleResult = await this.dbPool.query(`
      INSERT INTO automation_rules (id, name, account_id, automation_id, geofence_id, device_id, on_events, min_dwell_seconds, enabled)
